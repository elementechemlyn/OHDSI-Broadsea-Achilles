...
RUN R -e "options(timeout = 5); install.packages(c('remotes', 'ParallelLogger', 'SqlRender', 'DatabaseConnector'), quiet = TRUE, dependencies = TRUE, repos = 'http://cran.r-project.org')" && \
    R CMD javareconf && \
